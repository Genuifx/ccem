...
import { restoreComposerSessionReferences } from './composerSessionReferences';
...
export function toRecoveredComposerDraft(draft: RecoveryDraft) {
  ...
  const segments = restoreComposerSessionReferences(draft.text).flatMap((segment): Segment[] => {
    if (segment.type !== 'text') return [segment];
    return segment.text.split(/(\[Image #\d+\])/g).filter(Boolean).map((part): Segment => {
