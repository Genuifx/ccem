...
export type TriggerSuggestion = {
  ...
  icon?: ReactNode
  /** Optional badge labels rendered in suggestion rows */
  badges?: string[]
  /** Disabled suggestions are visible for diagnosis but cannot be selected */
  disabled?: boolean
  /** Optional arbitrary data passed through on selection */
  data?: unknown
}

export type PromptAreaTriggerPanelState = {
...
export type PromptAreaProps = {
  ...
  onEscape?: () => void
  /** Called when a chip element is clicked. Receives the chip's segment data and clicked element. */
  onChipClick?: (chip: ChipSegment, context: ChipClickContext) => void
  ...
  /** Called before plain text paste is inserted. Return true to handle it externally. */
  onBeforeTextPaste?: (text: string) => boolean
  /** Called before built-in key handling. Prevent default to stop PromptArea handling. */
  onKeyDown?: (event: KeyboardEvent<HTMLDivElement>) => void
  /** Called with dropdown trigger state when suggestions should be rendered by a parent surface. */
  onTriggerPanelChange?: (state: PromptAreaTriggerPanelState | null) => void
  /** Where to render trigger suggestions. Defaults to inline cursor-anchored popover. */
  triggerPanelPlacement?: 'inline' | 'external'
