...
export async function publishDraftGithubRelease({ client, desiredDraft, expectedAssets }) {
  if (desiredDraft !== true && desiredDraft !== false) fail('desired draft state must be boolean');
  if (!desiredDraft) {
    // Recheck the tag after every asset upload and immediately before the final
    // exact-draft read. GitHub does not offer a conditional/CAS release PATCH,
    // so the exact release GET must remain the final awaited request before it.
