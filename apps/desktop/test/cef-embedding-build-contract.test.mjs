...
test('windowed CEF close stays inside BrowserPanel and finalizes after AppKit exits', async () => {
  ...
  assert.match(pump, /drain_after_app_loop/,
    'CEF must perform its bounded post-AppKit drain before shutdown');
...
test('Windows CEF children start hidden when the current lease is hidden and close locally', async () => {
  const [surface, popup, util, pump, platformPump] = await Promise.all([
