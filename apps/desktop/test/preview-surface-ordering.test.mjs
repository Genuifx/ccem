...
test('Preview integration routes lifecycle mutations and active-session visibility through the shared lane', async () => {
  ...
  assert.match(
    panel,
    /usePreviewSurfaceMutation\([\s\S]*backend === 'preview' \? sessionId : null,[\s\S]*previewCloseRequestedRef,[\s\S]*\)/,
  );
  assert.doesNotMatch(panel, /previewVisibilityTailRef/);
  assert.match(panel, /runPreviewSurfaceMutation\(\(\) => invoke<BrowserOpenResponse>\('browser_open'/);
