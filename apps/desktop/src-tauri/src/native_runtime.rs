...
impl NativeRuntimeManager {
    ...
    fn commit_browser_permission_fields(
        ...
        let (previous_perm_mode, previous_runtime_perm_mode, workspace_dir) = {
            let record = handle
                .record
                .lock()
                .map_err(|_| "Failed to lock native session record".to_string())?;
        ...
        let sync_result =
            self.sync_login_browser_permission(app, &workspace_dir, next_ticket.clone());
    ...
    fn rollback_browser_permission_authority(
        ...
        {
            let _ = self.sync_login_browser_permission(app, workspace_dir, ticket);
        }
    }
    ...
    fn quarantine_permission_transition(
        ...
        match handle.browser_permission_sync.try_lock() {
            Ok(_permission_sync) => match handle.browser_permission.try_update("readonly") {
                Ok(ticket) => {
                    let workspace_dir = handle
                        .record
                        .try_lock()
                        .ok()
                        .map(|record| record.project_dir.clone());
                    match workspace_dir {
                        Some(workspace_dir) => {
                            if let Err(error) =
                                self.sync_login_browser_permission(app, &workspace_dir, ticket)
                            {
                                quarantine_errors.push(format!(
                        ...
                        None => quarantine_errors
                            .push("failed to read native runtime workspace".to_string()),
                    }
                }
        ...
        destroy_browser_session(Some(app), runtime_id);
        ...
        match output {
            HelperOutputEvent::SessionMeta {
                ...
                let (provider, pending_handoff_terminal, provider_session_id) =
                    self.bind_provider_session_lineage(runtime_id, &provider_session_id)?;
    ...
    fn handle_browser_tool_request(
        ...
        let response = match app {
            Some(app) => {
                ...
                match prepared {
                    Ok((_, browser_actor_id, _, _, Some((login, prepared)))) => {
                        login.execute_prepared_agent_tool(&browser_actor_id, &request, prepared)
                    }
