...
impl NativeRuntimeManager {
    ...
    pub fn update_session_settings(
        ...
        let delivery = if let Some(mode) = perm_mode {
            ...
            self.write_to_child_with_reconnect(app, runtime_id, Arc::clone(&handle), &command)
        };
    ...
    pub fn stop_session_from(
        ...
        self.stop_session_from_with_grace(Some(app), runtime_id, source, NATIVE_STOP_GRACE_PERIOD)
    }
    ...
    fn spawn_helper(
        ...
        if let Err(write_error) = self.write_to_child(
            ...
            return Err(match cleanup {
                Ok(_) => write_error,
                Err(cleanup_error) => {
                    format!("{write_error}; helper cleanup failed: {cleanup_error}")
                }
            });
        }
