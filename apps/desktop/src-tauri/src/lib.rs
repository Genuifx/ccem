...
fn stop_native_session(
    ...
    native_state.stop_session_from(&app, &runtime_id, source.as_deref())
}
...
#[cfg(test)]
fn desktop_context() -> tauri::Context<tauri::Wry> {
    panic!("desktop context is not constructed by unit tests")
}
...
pub fn run_desktop_app() -> i32 {
    ...
    match browser::login::cef::macos_safe_storage_smoke::gate_from_process_environment() {
        browser::login::cef::macos_safe_storage_smoke::MacosSafeStorageSmokeGate::Disabled => {}
        browser::login::cef::macos_safe_storage_smoke::MacosSafeStorageSmokeGate::Enabled(
            config,
        ) => {
            #[cfg(all(target_os = "macos", not(debug_assertions)))]
            return browser::login::cef::macos_safe_storage_smoke::run(*config, desktop_context());
            #[cfg(not(all(target_os = "macos", not(debug_assertions))))]
            {
                let _ = config;
                unreachable!("macOS Safe Storage smoke gate admitted an incompatible build");
            }
        }
        browser::login::cef::macos_safe_storage_smoke::MacosSafeStorageSmokeGate::Rejected(
            error,
        ) => {
            eprintln!(
                "{}",
                browser::login::cef::macos_safe_storage_smoke::rejection_json(&error)
            );
    ...
    match browser::login::cef::debug_smoke::gate_from_process_environment() {
        ...
        browser::login::cef::debug_smoke::MacosDebugMode2SmokeGate::Enabled(config) => {
            #[cfg(all(target_os = "macos", debug_assertions))]
            return browser::login::cef::debug_smoke::run(config, desktop_context());
            #[cfg(not(all(target_os = "macos", debug_assertions)))]
            {
                let _ = config;
                unreachable!("macOS debug smoke gate admitted an incompatible build");
            }
        }
        browser::login::cef::debug_smoke::MacosDebugMode2SmokeGate::Rejected(error) => {
            eprintln!(
                "{}",
                browser::login::cef::debug_smoke::rejection_json(&error)
            );
            return browser::login::cef::debug_smoke::EXIT_GATE_REJECTED;
        }
    }

    let windows_mode2_ci_smoke =
        match browser::login::cef::ci_smoke::gate_from_process_environment() {
            browser::login::cef::ci_smoke::WindowsMode2SmokeGate::Disabled => None,
            browser::login::cef::ci_smoke::WindowsMode2SmokeGate::Enabled(config) => Some(config),
            browser::login::cef::ci_smoke::WindowsMode2SmokeGate::Rejected(error) => {
                eprintln!("Windows Mode 2 production smoke rejected: {error}");
                return browser::login::cef::ci_smoke::EXIT_GATE_REJECTED;
            }
        };

    #[cfg(all(target_os = "windows", not(debug_assertions)))]
    let windows_mode2_ci_runtime = match windows_mode2_ci_smoke.as_ref() {
        Some(config) => match config.create_isolated_runtime() {
            Ok(runtime) => Some(runtime),
            Err(error) => {
                eprintln!("Windows Mode 2 production smoke failed: {error}");
                return browser::login::cef::ci_smoke::EXIT_SMOKE_FAILED;
            }
        },
        None => None,
    };
    ...
    let desktop_instance_lock = match desktop_instance_lock::acquire_desktop_instance_lock() {
        ...
        Err(error) => {
            eprintln!("{}", error);
            return if windows_mode2_ci_smoke.is_some() {
                browser::login::cef::ci_smoke::EXIT_SMOKE_FAILED
            } else {
                0
            };
        }
    ...
    let login_browser_session_manager = windows_mode2_ci_runtime
        ...
        .unwrap_or_else(browser::create_login_browser_session_manager);
    #[cfg(not(all(target_os = "windows", not(debug_assertions))))]
    let login_browser_session_manager = browser::create_login_browser_session_manager();
    #[cfg(all(target_os = "windows", not(debug_assertions)))]
    let login_browser_surface_manager = windows_mode2_ci_runtime
        .as_ref()
        .map(|runtime| runtime.surfaces.clone())
        .unwrap_or_else(|| {
            browser::create_login_browser_surface_manager(&login_browser_session_manager)
        });
    #[cfg(not(all(target_os = "windows", not(debug_assertions))))]
    let login_browser_surface_manager =
        browser::create_login_browser_surface_manager(&login_browser_session_manager);
    #[cfg(all(target_os = "windows", not(debug_assertions)))]
    let cef_host_controller = windows_mode2_ci_runtime
        .as_ref()
        .map(|runtime| runtime.cef_host.clone())
    ...
    let cef_host_controller_for_run = cef_host_controller.clone();
    #[cfg(any(target_os = "macos", windows))]
    let cef_host_controller_for_exit_prepare = cef_host_controller_for_run.clone();
    #[cfg(any(target_os = "macos", windows))]
    let login_browser_session_manager_for_exit = login_browser_session_manager.clone();
    #[cfg(any(target_os = "macos", windows))]
    let login_browser_surface_manager_for_exit = login_browser_surface_manager.clone();
    #[cfg(any(target_os = "macos", windows))]
    let cef_exit_prepare_attempted = Arc::new(AtomicBool::new(false));
    #[cfg(any(target_os = "macos", windows))]
    let cef_exit_prepare_attempted_for_run = cef_exit_prepare_attempted.clone();
    #[cfg(all(target_os = "macos", debug_assertions))]
    let cef_host_controller_for_setup = cef_host_controller.clone();
    #[cfg(any(
        all(target_os = "macos", debug_assertions),
        all(target_os = "windows", not(debug_assertions))
    ))]
    let cef_host_controller_for_page_load = cef_host_controller.clone();
    #[cfg(all(target_os = "windows", not(debug_assertions)))]
    let windows_mode2_ci_smoke_for_page_load = windows_mode2_ci_smoke.clone();
    #[cfg(all(target_os = "windows", not(debug_assertions)))]
    let windows_mode2_ci_runtime_for_page_load = windows_mode2_ci_runtime.clone();
    #[cfg(all(target_os = "windows", not(debug_assertions)))]
    let browser_manager_for_windows_smoke = browser_manager.clone();
    ...
    let app = builder
        .on_page_load(move |webview, payload| {
            ...
            if let Some(config) = windows_mode2_ci_smoke_for_page_load.as_deref().cloned() {
                ...
                let prepare_window = main_window
                    .show()
                    .map_err(|error| format!("show Windows Mode 2 smoke host window: {error}"))
                    .and_then(|()| {
                        main_window.set_focus().map_err(|error| {
                            format!("focus Windows Mode 2 smoke host window: {error}")
                        })
                    });
                if let Err(error) = prepare_window {
                    eprintln!("Windows Mode 2 production smoke failed: {error}");
                    smoke_app.exit(browser::login::cef::ci_smoke::EXIT_SMOKE_FAILED);
                    return;
                }
                if let Err(error) = browser::login::cef::ci_smoke::spawn(
                    smoke_app.clone(),
                    windows_mode2_ci_runtime_for_page_load
                        .as_ref()
                        .expect("enabled Windows Mode 2 smoke has an isolated runtime")
                        .clone(),
                    browser_manager_for_windows_smoke.clone(),
                    config,
                ) {
                    eprintln!("Windows Mode 2 production smoke failed: {error}");
                    smoke_app.exit(browser::login::cef::ci_smoke::EXIT_SMOKE_FAILED);
            ...
            #[cfg(all(target_os = "macos", debug_assertions))]
            if std::env::var("CCEM_CEF_SURFACE_SMOKE").as_deref() == Ok("1") {
                use browser::login::cef::surface::{CefSurfaceRequest, LogicalViewport};
                use std::io::{Read, Write};
                use std::time::{Duration, Instant};

                const SMOKE_URL: &str = concat!(
                    "data:text/html,%3Cmeta%20charset%3Dutf-8%3E%3Ctitle%3ECCEM_CEF_TAURI_CHILD_READY%3C%2Ftitle%3E",
                    "%3Cstyle%3Ehtml%2Cbody%7Bheight%3A100%25%3Bmargin%3A0%7Dbody%7Bdisplay%3Agrid%3Bplace-items%3Acenter%3B",
                    "background%3A%230b1220%3Bcolor%3A%23e7f7ff%3Bfont%3A700%2030px%20system-ui%7D",
                    ".card%7Bpadding%3A38px%2048px%3Bborder%3A2px%20solid%20%235eead4%3Bborder-radius%3A20px%7D%3C%2Fstyle%3E",
                    "%3Cdiv%20class%3Dcard%3EMODE%202%20%C2%B7%20TAURI%20CEF%20CHILD%3C%2Fdiv%3E"
                );
                let request = CefSurfaceRequest {
                    surface_id: "mode2-tauri-smoke".to_string(),
                    profile_id: "profile-00000000000000000000000000000001".to_string(),
                ...
                let smoke_surface_id = request.surface_id.clone();
                let smoke_app = webview.app_handle().clone();
                let smoke_controller = cef_host_controller_for_page_load.clone();
                match cef_host_controller_for_page_load.open_surface(webview.app_handle(), request) {
                    Ok(mut connection) => {
                        let _ = std::thread::Builder::new()
                            .name("ccem-cef-surface-smoke".to_string())
                            .spawn(move || {
                                match connection.wait_until_ready(Duration::from_secs(12)) {
                                    Ok(snapshot) => eprintln!(
                                        "CEF Tauri child smoke ready surface={} url={}",
                                        snapshot.surface_id, snapshot.current_url
                                    ),
                                    Err(error) => {
                                        eprintln!("CEF Tauri child smoke failed: {error}");
                                        return;
                                    }
                                }
                                let mut command = br#"{"id":91001,"method":"Runtime.evaluate","params":{"expression":"({title:document.title,href:location.href})","returnByValue":true}}"#.to_vec();
                                command.push(0);
                                if let Err(error) = connection.writer.write_all(&command) {
                                    eprintln!("CEF Tauri child smoke CDP write failed: {error}");
                                    return;
                                }
                                    ...
                                    match connection.reader.read(&mut chunk) {
                                        ...
                                        Ok(count) => {
                                            ...
                                            if frame.last() == Some(&0) {
                                                ...
                                                let is_expected_document = response
                                                    .as_ref()
                                                    .filter(|value| value.get("id").and_then(|id| id.as_i64()) == Some(91001))
                                                    .is_some_and(|value| {
                                                        value.pointer("/result/result/value/title").and_then(|title| title.as_str())
                                                            == Some("CCEM_CEF_TAURI_CHILD_READY")
                                                            && value.pointer("/result/result/value/href").and_then(|href| href.as_str())
                                                                .is_some_and(|href| href.starts_with("data:text/html,"))
                                                    });
                                                if is_expected_document {
                                                    eprintln!(
                                                        "CEF Tauri child smoke CDP ready bytes={} payload={}",
                                                        payload.len(),
                                        ...
                                        Err(error) => {
                                            eprintln!("CEF Tauri child smoke CDP read failed: {error}");
                                            break;
                                        }
                                    }
                                }
                                if !cdp_ready {
                                    eprintln!("CEF Tauri child smoke CDP did not reach the requested document");
                                }
                                let mut target_info_command = br#"{"id":91002,"method":"Target.getTargetInfo","params":{}}"#.to_vec();
                                target_info_command.push(0);
                                if let Err(error) = connection.writer.write_all(&target_info_command) {
                                    eprintln!("CEF Tauri child smoke target-info write failed: {error}");
                                    return;
                                }
                                let target_info_deadline = Instant::now() + Duration::from_secs(5);
                                let mut target_info_frame = Vec::new();
                                let mut target_info_ready = false;
                                while Instant::now() < target_info_deadline {
                                    let mut chunk = [0_u8; 4096];
                                    match connection.reader.read(&mut chunk) {
                                        Ok(0) => break,
                                        Ok(count) => {
                                            target_info_frame.extend_from_slice(&chunk[..count]);
                                            if target_info_frame.last() == Some(&0) {
                                                let payload = &target_info_frame[..target_info_frame.len() - 1];
                                                let response = serde_json::from_slice::<serde_json::Value>(payload).ok();
                                                if response
                                                    .as_ref()
                                                ...
                                                {
                                                    eprintln!(
                                                        "CEF Tauri child smoke target-info payload={}",
                                                        String::from_utf8_lossy(payload)
                                                    );
                                                    target_info_ready = true;
                                                    break;
                                                }
                                                target_info_frame.clear();
                                            }
                                        }
                                        Err(error) if error.kind() == std::io::ErrorKind::TimedOut => {}
                                        Err(error) => {
                                            eprintln!("CEF Tauri child smoke target-info read failed: {error}");
                                            break;
                                        }
                                    }
                                }
                                if !target_info_ready {
                                    eprintln!("CEF Tauri child smoke target-info did not return");
                                }
                                let interaction_result = (|| -> Result<(), String> {
                                    smoke_controller.set_surface_viewport(
                                        &smoke_app,
                                        smoke_surface_id.clone(),
                                        LogicalViewport {
                                            x: 170.0,
                                            y: 130.0,
                                            width: 620.0,
                                    ...
                                    smoke_controller.set_surface_visible(
                                        &smoke_app,
                                        smoke_surface_id.clone(),
                                        false,
                                    )?;
                                ...
                                match interaction_result {
                                    Ok(()) => eprintln!(
                                        "CEF Tauri child smoke interaction ready resize=true hide_show=true focus=native_click_required"
                                    ),
                                    Err(error) => eprintln!(
                                        "CEF Tauri child smoke interaction failed: {error}"
                                    ),
                                }
                                if let Err(error) = smoke_controller.close_surface(
                                    &smoke_app,
                                    smoke_surface_id.clone(),
                                ) {
                                    eprintln!("CEF Tauri child smoke close command failed: {error}");
                                    return;
                                }
                                match connection.wait_until_closed(Duration::from_secs(10)) {
                                    Ok(snapshot) => eprintln!(
                                        "CEF Tauri child smoke closed surface={} lifecycle={:?}",
                                        snapshot.surface_id, snapshot.lifecycle
                                    ),
                                    Err(error) => {
                                        eprintln!("CEF Tauri child smoke close failed: {error}");
                                        return;
                                    }
                                }

                                let reopen_request = CefSurfaceRequest {
                                    surface_id: smoke_surface_id.clone(),
                                    profile_id: "profile-00000000000000000000000000000001".to_string(),
                                    initial_url: SMOKE_URL.to_string(),
                                    viewport: LogicalViewport {
                                        x: 190.0,
                                ...
                                let reopened = match smoke_controller
                                    .open_surface(&smoke_app, reopen_request)
                                {
                                    Ok(connection) => connection,
                                    Err(error) => {
                                        eprintln!("CEF Tauri child smoke reopen failed: {error}");
                                        return;
                                    }
                                };
                                match reopened.wait_until_ready(Duration::from_secs(12)) {
                                    Ok(snapshot) => eprintln!(
                                        "CEF Tauri child smoke reopened surface={} url={}",
                                        snapshot.surface_id, snapshot.current_url
                                    ),
                                    Err(error) => {
                                        eprintln!("CEF Tauri child smoke reopen load failed: {error}");
                                        return;
                                    }
                                }
                                if let Err(error) = smoke_controller.close_surface(
                                    ...
                                    eprintln!("CEF Tauri child smoke reopen close command failed: {error}");
                                    return;
                                }
                                match reopened.wait_until_closed(Duration::from_secs(10)) {
                                    Ok(snapshot) => eprintln!(
                                        "CEF Tauri child smoke reclosed surface={} lifecycle={:?}",
                                        snapshot.surface_id, snapshot.lifecycle
                                    ),
                                    Err(error) => eprintln!(
                                        "CEF Tauri child smoke reopen close failed: {error}"
                                    ),
                                }
                                drop(reopened);
                                drop(connection);
                    ...
                    Err(error) => eprintln!("CEF Tauri child smoke open failed: {error}"),
                }
            }
        })
        ...
        .setup(move |app| {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                // Explicit development/runtime gate. Normal app startup remains Wry-only;
                // production Mode 2 initializes lazily when its embedded surface opens.
                #[cfg(debug_assertions)]
                if std::env::var("CCEM_CEF_INIT_SMOKE").as_deref() == Ok("1") {
                    match cef_host_controller_for_setup.ensure_ready(app.handle()) {
                        Ok(layout) => eprintln!(
                            "CEF initialization smoke ready bundled={} framework={}",
                            layout.bundled,
                            layout.framework_path.display()
                        ),
                        Err(error) => eprintln!("CEF initialization smoke failed: {error}"),
                    }
                }
            }
    ...
    #[cfg(any(target_os = "macos", windows))]
    if let Err(error) = cef_host_controller_for_run.finish_shutdown_current_thread() {
        eprintln!("CEF shutdown finalization warning: {error}");
    }
    exit_code
}
