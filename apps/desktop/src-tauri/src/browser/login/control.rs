...
impl CancellationState {
    ...
    fn update_and_wake<T>(&self, update: impl FnOnce(&ExecutionFence) -> T) -> T {
        // Pair the predicate transition with the same mutex used by `wait_cancelled`.
        // Otherwise a notifier can advance the fence after the wait predicate is checked but
        // before the Condvar actually sleeps, losing the only wake-up.
        let _waiters = self
            .wait_lock
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let result = update(&self.fence);
        self.wake.notify_all();
        result
    }

    fn cancel(&self) -> RetiredOperationEpoch {
        let retired = self.update_and_wake(ExecutionFence::retire_current);
...
impl EffectSafetyFence {
    pub(super) fn mark_unconfirmed(&self) {
        self.state
            .update_and_wake(|fence| fence.mark_effect_unsafe(self.generation));
    }
}
