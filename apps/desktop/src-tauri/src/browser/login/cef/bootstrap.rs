...
impl CefProcess {
    ...
    pub(crate) fn prepare_shutdown(&mut self) -> Result<(), String> {
        ...
        self.pump.drain_before_app_loop_exit()?;
    ...
    pub(crate) fn finish_shutdown(mut self) -> Result<(), String> {
        if self.initialized {
            ...
            // The bounded native/CEF drain already ran from ExitRequested while Tao's event
            // callback was alive. Never pump CFRunLoop after run_return has destroyed it.
            self.pump.stop();
            cef::shutdown();
