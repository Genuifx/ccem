...
pub(crate) fn create_surface(
    ...
    let Some(context) = context else {
        SURFACES.with(|surfaces| {
            surfaces.borrow_mut().remove(&spec.surface_id);
        });
        shared.fail_creation("CEF failed to allocate an isolated RequestContext");
        return Err("CEF failed to allocate an isolated RequestContext".to_string());
    };
    SURFACES.with(|surfaces| {
...
pub(crate) fn shutdown_all(pump: &CefExternalPump) -> Result<(), String> {
    ...
    if !all_surfaces_closed() {
        ...
        return Err(format!(
            "CEF surfaces did not close before the shutdown deadline: {remaining}"
        ));
    }
