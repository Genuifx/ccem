...
// CEF's external-pump sample has no public "queue is idle" signal, so shutdown advances the
// native loop and CEF work ten times with a 50 ms settle. macOS must do this before Tao destroys
// its callback; Windows retains the existing post-run-loop drain.
...
impl CefExternalPump {
    ...
    /// Complete the bounded drain used by CEF's macOS external-pump sample before the native
    /// application loop callback is released and before `cef_shutdown`.
    ///
    /// Tao drops its event callback before `run_return` returns. Pumping CFRunLoop after that
    /// point can dispatch into a destroyed callback, so this method must run from the
    /// `ExitRequested` prepare phase while the app event loop is still alive.
    #[cfg(target_os = "macos")]
    pub(crate) fn drain_before_app_loop_exit(&self) -> Result<(), String> {
        if self.state.phase() != PumpPhase::Draining {
            return Err("CEF pre-exit drain requires the draining phase".to_string());
        }
        ...
        for _ in 0..POST_RUN_LOOP_DRAIN_ITERATIONS {
            self.state
                .platform
                .lock()
                .map_err(|_| "CEF platform pump is unavailable during pre-exit drain".to_string())?
    ...
    /// Windows keeps its bounded drain after the native application loop returns and before
    /// `cef_shutdown`; unlike AppKit, this does not dispatch through a destroyed Tao callback.
    #[cfg(windows)]
