...
impl LoginBrowserSessionManager {
    /// Route an existing native Agent browser request to Mode 2 only when the trusted UI has an
    /// active handoff for the same canonical workspace. `None` means Preview Browser remains the
    /// selected backend. Agent arguments never select a profile/session/backend directly.
    pub(crate) fn prepare_agent_tool_if_handed_off(
        ...
        let Some(lease) = self
            .agent_execution_for_workspace(&workspace)
            .map_err(|error| error.to_string())?
        else {
            return Ok(None);
        };
        ...
        let permission = self
            .update_permission_for_agent_lease(&lease, authority)
            .map_err(|error| error.to_string())?;
    ...
    pub(crate) fn run_agent_tool_if_handed_off(
        ...
        prepared
            .map(|prepared| self.execute_prepared_agent_tool(actor_id, request, prepared))
            .transpose()
    }
}
