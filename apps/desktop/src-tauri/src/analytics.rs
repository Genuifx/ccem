...
// Version 3 adds per-file parse continuation state to `CacheFileEntry`
// (`parseOffset`, `lastLineComplete`, `codexState`). Caches stamped with an
// older version are discarded on read and rebuilt from scratch (one-time
// full parse after upgrade).
...
// Version-3 entry shape: besides `meta` + `stats`, each entry carries parse
...
//   between incremental parses. Claude parsing is stateless and stores None.
...
/// version-3 entry shape comment above).
#[derive(Debug, Serialize, Deserialize, Clone, Default, PartialEq)]
...
fn parse_claude_jsonl_reader<R: BufRead>(
    ...
    for line in reader.lines().map_while(Result::ok) {
        accumulate_claude_line(&mut entries, &line, prices);
    }
    CacheStats { entries }
}
...
struct JsonlTailStats {
    /// Entries collected from the lines consumed in this pass only.
    stats: CacheStats,
...
fn parse_jsonl_tail(
    ...
    for (index, byte) in bytes.iter().enumerate() {
        ...
        if let Ok(line) = std::str::from_utf8(&bytes[line_start..index]) {
            match source {
                UsageSource::Claude => accumulate_claude_line(&mut entries, line, prices),
                UsageSource::Codex => accumulate_codex_line(&mut state, &mut entries, line, prices),
            }
        }
        line_start = index + 1;
    }
...
fn full_parse_jsonl(
    ...
    let tail = parse_jsonl_tail(source, &bytes, 0, CodexParseState::default(), prices);
    codex_aware_entry(
        meta,
        tail.stats,
...
fn incremental_refresh_entry(
    ...
    stats.entries.extend(tail.stats.entries);

    let entry = codex_aware_entry(
        meta.clone(),
        stats,
        tail.consumed_offset,
        tail.last_line_complete,
...
mod tests {
    use super::{
        ...
        CacheEntry, CacheFile, CacheFileEntry, CacheMeta, CacheStats, CacheUsage, CodexParseState,
        DiscoveredFile, ModelBreakdownGranularity, ModelPrice, UsageSource, UsageStats,
