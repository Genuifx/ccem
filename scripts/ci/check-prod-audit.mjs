  ...
  const visit = (dependencies, pathSegments, depth) => {
    ...
    for (const [dependencyName, node] of entries) {
      ...
      if (typeof node.path !== 'string' || !path.isAbsolute(node.path)) {
        throw new Error(`pnpm list omitted the absolute package path for ${dependencyName}.`);
      }
...
export function decodeBulkHttpResponse({ statusCode, headers = {}, body }) {
  if (statusCode !== 200) {
    throw new Error(`npm bulk audit returned HTTP ${statusCode ?? 'unknown'}.`);
  }
...
export function requestBulkAdvisories(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      fail(new Error(`npm bulk audit timed out after ${timeoutMs}ms.`));
    }, timeoutMs);
    ...
    try {
      request = requestImpl(
        ...
        response => {
          ...
          response.on('aborted', () => {
            fail(new Error('npm bulk audit response was aborted.'));
          });
          response.on('error', error => {
            fail(new Error(`npm bulk audit response failed: ${error.message}`));
          });
          response.on('end', () => {
            if (settled) {
              return;
      ...
      fail(new Error(`npm bulk audit request failed: ${error.message}`));
      return;
    }
...
async function main() {
  ...
  const report = await auditProductionDependencies(dependencyIndex);
